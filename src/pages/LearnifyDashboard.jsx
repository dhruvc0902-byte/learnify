...
const SignInModal = ({ onClose, onAuthSuccess }) => {
...
console.log("bcrypt functions:", bcrypt);
console.log("Password chars:", [...password]);
  ...
  setPassword(e.target.value);
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold rounded-xl hover:shadow-xl hover:shadow-purple-400/40 transition disabled:opacity-60"
          >
...
const SignUpModal = ({ onClose, onAuthSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (roleError || !roleData) {
        setErrMsg("Roles table not configured correctly.");
        setLoading(false);
        return;
      }
      ...
      const { data: user, error } = await supabase
        ...
        .select("id, fullname, email, mobile, role_id, roles(name)")
        .single();
      ...
      if (error) {
        if (error.code === "23505") {
          ...
          setErrMsg(error.message || "Failed to create account.");
        }
        setLoading(false);
        return;
      }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-xl hover:shadow-xl hover:shadow-pink-400/40 transition disabled:opacity-60"
          >
...
/* --------------- PROFILE MODAL (edit fullname + password) --------------- */

const ProfileModal = ({ onClose, currentUser, onUpdated }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (error || !data) {
        setErrMsg(error?.message || "Failed to update profile");
        setLoading(false);
        return;
      }
      ...
      const updatedUser = {
        ...
        role_name: data.roles?.name || currentUser.role_name,
      };
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-500 mb-4 text-center">
          You can change your <b>full name</b> and <b>password</b> only.
        </p>
        ...
        <form onSubmit={handleUpdate} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="password"
              className="w-full p-3 rounded-xl border border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none transition"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold rounded-xl hover:shadow-lg transition disabled:opacity-60"
          >
                ...
                {videos.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {videos.map((v, idx) => (
                      <li
                        key={v.id}
                        className={`flex items-start gap-2 p-2 rounded-xl cursor-pointer text-xs ${
                          activeVideo?.id === v.id
...
const AdminPanel = ({ onDataChanged }) => {
  ...
  // Categories from Supabase `categories`
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewCourse((p) => ({ ...p, students: e.target.value }))
                    }
                  />
                </div>
                <div>
              ...
              <button
                type="submit"
                className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg font-semibold text-sm hover:shadow-lg disabled:opacity-60"
...
export default function LearnifyHero() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("learnifyUser");
    if (stored) {
      try {
  ...
  setCurrentUser(user);
  localStorage.setItem("learnifyUser", JSON.stringify(user));
  ...
  const handleSignOut = () => {
    setCurrentUser(null);
    localStorage.removeItem("learnifyUser");
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
                ...
                <>
                  <div className="hidden sm:flex flex-col items-end">
                    <span className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                      ...
                      {isAdmin && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] bg-purple-100 text-purple-700 border border-purple-200">
                          Admin
                        </span>
                      )}
                    </span>
                  ...
                  <button
                    className="hidden sm:inline-block px-4 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-semibold hover:bg-gray-50"
            ...
            <div className="mt-4 flex flex-col gap-3 md:hidden">
              ...
              <div className="flex flex-col gap-2 pt-2">
                {currentUser ? (
                  <>
                    <button
                      className="px-4 py-2.5 rounded-xl border border-gray-300 font-semibold text-gray-700 text-left"
                      onClick={() => {
                        setShowProfile(true);
                        setMobileNavOpen(false);
                      }}
                    ...
                    <button
                      className="px-4 py-2.5 rounded-xl bg-red-500 text-white font-semibold text-left"
                      onClick={() => {
                        handleSignOut();
                        setMobileNavOpen(false);
                      }}
                  ...
                  <>
                    <button
                      className="px-4 py-2.5 rounded-xl border border-gray-300 font-semibold text-gray-700"
                      onClick={() => {
                        setShowSignIn(true);
                        setMobileNavOpen(false);
                      }}
      ...
      {/* Hero Section */}
      {/* (unchanged hero, features, paths, courses, testimonials, footer) */}
      {/* --- I’ll keep your existing UI, only updated where necessary (tags rendering etc.) --- */}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {filteredCourses.length === 0 ? (
            <p className="text-center text-gray-500">
              No courses yet. {isAdmin ? "Use the Admin Panel below to create some." : "Please check back later."}
            </p>
          ) : (
